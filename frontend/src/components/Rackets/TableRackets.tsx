...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/rackets/racketsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureRacketsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListRackets from './ListRackets';

const perPage = 10;

const TableSampleRackets = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    loading,
    count,
    notify: racketsNotify,
    refetch,
  } = useAppSelector((state) => state.rackets);
  ...
  useEffect(() => {
    if (racketsNotify.showNotification) {
      notify(racketsNotify.typeNotification, racketsNotify.textNotification);
    }
  }, [racketsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/rackets/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/rackets/rackets-view/?id=${id}`);
  };
...
export default TableSampleRackets;
