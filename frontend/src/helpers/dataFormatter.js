...
export default {
  ...
  racketsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.model);
  },
  racketsOneListFormatter(val) {
    if (!val) return '';
    return val.model;
  },
  racketsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.model };
    });
  },
  racketsOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.model, id: val.id };
  },
